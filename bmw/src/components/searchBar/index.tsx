  ...
  return (
    <form onSubmit={handleSubmit} className="searchbar gap-3">
      <div className="searchbar__item">
        <ReactSelect defaultValue={selected} options={options} placeholder="Marka Seçiniz" className="w-full text-black" onChange={(e) => e && setMake(e.value || "")}/>
        <Button designs="sm:hidden"/>
      </div>
      <div className="searchbar__item">
        <img src="/model-icon.png" className="absolute ml-4" width={25}/>
        <input defaultValue={params.get("model") || ""} type="text" className="searchbar__input rounded text-black" placeholder="örn:Civic" onChange={(e) => setModel(e.target.value)}/>
        <Button/>
      </div>
    </form>
  )
